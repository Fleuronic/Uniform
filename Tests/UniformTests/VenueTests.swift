...
struct VenueTests {
	@Test func matchesLegacy() {
		let inputs: [(record: String, address: String)] = [
			("x", "1 Bills Dr"), ("x", "4300 Main St"), ("x", "1500 Greenland Dr"),
			("x", "1620 E Bulldog Ln"), ("x", "114 17th Ave East"), ("x", "600 Lincoln Ave"),
			("x", "Rte 111-West Hollis St"), ("x", "not a known address"),
			("Central High School Stadium at Central High School", "zzz"),
			("Orrville High School Stadium", "zzz"),
			("Foshee", "zzz"),
			("Some Field at Texas A&amp;M", "zzz"),
			("Some Field at West Texas A&amp;M", "zzz"),
			("Alta High School", "zzz"),
			("Foo HS Stadium at Foo High School", "zzz"),
			("Tiger Stadium", "zzz"),
			("Cedarburg High School Stadium", "zzz"),
			("Salem Stadium", "zzz"),
			("Vikings Stadium", "zzz"),
			("O'Brien  Field", "zzz"),
			("Random Field at Some College", "zzz"),
			("Monterey Trail High School Stadium", "zzz")
		]

		for input in inputs {
			#expect(
				Venue.info(for: input.record, at: input.address) == Self.legacyInfo(for: input.record, at: input.address),
				"mismatch for \(input.record) / \(input.address)"
			)
		}
	}

	private static func legacyInfo(for record: String, at streetAddress: String) -> (String, String?) {
